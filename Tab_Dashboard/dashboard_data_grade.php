...
$sql = $mysqli->query("
    SELECT 'Clean' AS Result, COUNT(us.Student_ID) AS Count
	FROM user_student us
    	INNER JOIN curriculum c ON us.Curriculum_ID=c.Curriculum_ID
        INNER JOIN program p ON c.Program_ID=p.Program_ID
	WHERE (SELECT g2.Student_ID FROM grades g2 WHERE g2.Grade > 3 AND g2.Student_ID=us.Student_ID LIMIT 1) IS Null
    	$sql_usertype
	UNION ALL
	SELECT 'Inc' AS Result, COUNT(us.Student_ID) AS Count
	FROM user_student us
    	INNER JOIN curriculum c ON us.Curriculum_ID=c.Curriculum_ID
        INNER JOIN program p ON c.Program_ID=p.Program_ID
	WHERE (SELECT g2.Student_ID FROM grades g2 WHERE g2.Grade = 4 AND g2.Student_ID=us.Student_ID LIMIT 1)
    	$sql_usertype
	UNION ALL
	...
	WHERE (SELECT g2.Student_ID FROM grades g2 WHERE g2.Grade = 6 AND g2.Student_ID=us.Student_ID LIMIT 1)
    	$sql_usertype
	UNION ALL
	SELECT 'Failed' AS Result, COUNT(us.Student_ID) AS Count
	FROM user_student us
    	INNER JOIN curriculum c ON us.Curriculum_ID=c.Curriculum_ID
        INNER JOIN program p ON c.Program_ID=p.Program_ID
	WHERE (SELECT g2.Student_ID FROM grades g2 WHERE g2.Grade = 5 AND g2.Student_ID=us.Student_ID LIMIT 1)
    	$sql_usertype
");
